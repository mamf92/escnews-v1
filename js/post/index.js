...
function createAdminCardSmall(post) {
    ...
    deleteButton.addEventListener('click', function () { deletePostWithAuthorization(post.id) });


    const viewButton = document.createElement('button');
    viewButton.classList.add('button', 'primary', 'view', 'admin-cta');
    viewButton.addEventListener('click', function () { routeToPostWithID(post.id) });

    const editButton = document.createElement('button');
    editButton.classList.add('button', 'primary', 'edit', 'admin-cta');
    editButton.addEventListener('click', function () { routeToEditWithID(post.id) });
...
function displayLoadMorePostsButton(posts) {
    const morePublishedPostsSection = document.querySelector('.more-published-posts');
    const visiblePosts = document.querySelectorAll('.card-xs , .card-m, .card-tn');
    ...
    if (visiblePosts.length <= totalNumberOfPosts) {
        ...
        loadMorePostsButton.classList.add('button', 'primary', 'arrow-down', 'small', 'load-more-posts-button');
        loadMorePostsButton.addEventListener('click', () => { loadMorePosts(posts); })
...
function loadMorePosts(posts) {
    const morePublishedPostsContent = document.querySelector('.more-published-posts__content');
    const visiblePosts = document.querySelectorAll('.admin-card-s , .admin-card-tn');
    ...
    if (lastPostToLoad >= posts.length) {
        const loadMorePostsButton = document.querySelector('.load-more-posts-button');
